  ...
  try {
    ...
    if (req.user.role === 'admin') {
      ...
      if (active_only === 'true') {
        ...
        reminders = await ReminderModel.findAll(parseInt(limit as string as string), parseInt(offset as string as string));
      }
    } else {
...
router.get('/:id', authenticateToken, async (req: Request & {user?: any}, res: Response) => {
  try {
    const { id } = req.params;
    const reminder = await ReminderModel.findById(parseInt(id as string));
...
router.put('/:id', authenticateToken, async (req: Request & {user?: any}, res: Response) => {
  try {
    ...
    const reminder = await ReminderModel.update(parseInt(id as string), updates);
...
router.delete('/:id', authenticateToken, async (req: Request & {user?: any}, res: Response) => {
  try {
    const { id } = req.params;
    const deleted = await ReminderModel.delete(parseInt(id as string));
...
router.get('/customer/:customerId', authenticateToken, async (req: Request & {user?: any}, res: Response) => {
  try {
    ...
    const reminders = await ReminderModel.findByCustomerId(parseInt(customerId as string));
    res.json({ reminders });
  } catch (error) {
...
// Get due reminders
router.get('/due/now', authenticateToken, async (req: Request & {user?: any}, res: Response) => {
  try {
...
router.patch('/:id/notified', authenticateToken, async (req: Request & {user?: any}, res: Response) => {
  try {
    const { id } = req.params;
    const success = await ReminderModel.markAsNotified(parseInt(id as string));
